 ...
 * @author wind
 */
public class ComputerMonitorUtils {
    ...
    public static double getCpuInfo(){
        if(isWindow()){
            ...
            return NumberUtils.getDouble((total / totalidle) * 100, 2);
        }
    }
    ...
    public static float getNetInfo(){
        ...
        if(!isWindow()){
            ...
            if(in1 != 0 && in2 != 0 && out1 != 0 && out2 != 0){
                ...
                netUsage = curRate/Const.TotalBandwidth;
            }
    ...
    public static MemInfo getMemInfo(){
        ...
        if(isWindow()){
            ...
            memInfo = new MemInfo(osmxb.getTotalPhysicalMemorySize(), free, NumberUtils.getPercent(free, total));
        }else{
            List<String> lists = ProcessUtils.getProcessInfo(Const.LINUX_MEM_PROC);
            if(lists != null && lists.size() > 0){
                ...
                double percent = NumberUtils.getDouble((total - free - buffers - cached) / total * 100, 2);
    ...
    public static DiskInfo getDiskInfo(){
        ...
        if(isWindow()){
            for (char c = Const.CH_A; c <= Const.CH_Z; c++) {
                ...
                if (win.exists()) {
                    ...
                    double percent = NumberUtils.getPercent(free, total);
            ...
            List<String> lists = ProcessUtils.getProcessInfo(Const.LINUX_DISK_PROC);
            if (lists != null && lists.size() > 0) {
                int count = -1;
                for (String str : lists) {
                    String[] strArray = StringUtils.split(str, Const.STR_SPACE);
                    if (++count > 0) {
        ...
        DiskInfo diskInfo = new DiskInfo(allTotal, allFree, NumberUtils.getPercent(allFree, allTotal), "all");
    ...
    public static float getIoRatio(){
        float ioUsage = 0.0f;
        if(!isWindow()){
            List<String> lists = ProcessUtils.getProcessInfo(Const.LINUX_IO_PROC);
    ...
    private static Map<String, Long> readWinCpuInfo(){
        ...
        List<String> lists = ProcessUtils.getProcessInfo(Const.WIN_CPU_PROC);
        if(lists != null && lists.size() > 0){
            long idletime = 0;
            //读取物理设备时间
            long kneltime = 0;
    ...
    private static Map<String, String> readLinuxCpuInfo(){
        List<String> lists = ProcessUtils.getProcessInfo(Const.LINUX_CPU_PROC);
    ...
    private static Map<String, Long> readNetInfo(){
        ...
        List<String> lists = ProcessUtils.getProcessInfo(Const.LINUX_NET_PROC);
        if(lists != null && lists.size() > 0){
            for(String line : lists){
                line = line.trim();
